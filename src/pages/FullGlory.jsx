...
export default function FullGlory() {
  ...
  const stopAndReset = useCallback(() => {
    ...
    if (audio) {
      audio.pause();
      audio.src = LIVE_TRACKS[0].audioSrc;
      audio.load();
      audio.volume = 1;
    }
    setIsPlaying(false);
  ...
  // Init audio element
  useEffect(() => {
    const audio = new Audio(LIVE_TRACKS[0].audioSrc);
    audio.loop = false;
    audio.preload = 'auto';
    ...
    const handleEnded = () => {
      ...
      const nextIdx = (currentIdxRef.current + 1) % LIVE_TRACKS.length;
      clearInterval(loopFadeRef.current);
      loopFadingRef.current = false;
      audio.src = LIVE_TRACKS[nextIdx].audioSrc;
      audio.load();
  ...
  const track = LIVE_TRACKS[currentIdx];
  const showReadyState = !isPlaying && timeLeft === TOTAL_SECONDS;
  ...
  return (
    <div className="player">
      ...
      <div className="player__body">
        ...
        <p className="player__journey-label">full glory · 30 min</p>
        ...
        <p className="player__status">
          {isFading ? 'fading out...' : isPlaying ? 'cycling' : ''}
        </p>
      </div>
    </div>
  );
